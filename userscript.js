...
function addPositiveUI() {
  ...
  if (
    $("#downvote").find("i").attr("class") == "dislike-icon like-item voted"
  ) {
    removeNegativeUI();
  }
}
...
(function () {
  ...
  const userID = RegExp(/XID=(\d+)/).exec(
    $(".settings-menu").find("a").attr("href")
  )[1];
  if ($(location).attr("href").includes("profiles.php")) {
    targetID = RegExp(/XID=(\d+)/).exec($(location).attr("href"))[1];
  }
  let credibilityBar = `<div class="vote-bar">
